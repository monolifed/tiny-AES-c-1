...
// can be computed as AESxxx_KEYLEN / 4
#define AES128_NK 4
#define AES192_NK 6
#define AES256_NK 8

// can be computed as AESxxx_NK + 6
#define AES128_NR 10
#define AES192_NR 12
#define AES256_NR 14

static const uint32_t aes_nr[] = {AES128_NR, AES192_NR, AES256_NR};
static const uint32_t aes_nk[] = {AES128_NK, AES192_NK, AES256_NK};
...
void AES_CBC_decrypt_buffer(AES_ctx *ctx, uint8_t *buf,  uint32_t length)
{
	...
	for (i = 0; i < length; i += AES_BLOCKLEN)
	{
		memcpy(ctx->buffer, buf, AES_BLOCKLEN);
		InvCipher(ctx, (state_t) buf);
		XorWithIv(buf, ctx->Iv);
		memcpy(ctx->Iv, ctx->buffer, AES_BLOCKLEN);
		buf += AES_BLOCKLEN;
	}
